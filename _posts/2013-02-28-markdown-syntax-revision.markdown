...
Markdown不是要取代HTML，甚至连接近都算不上，它的语法种类很少，只对应HTML的一部分。虽然Markdown经过处理后是变成HTML文档，但Mardown的构想不是使HTML容易书写，HTML已经很容易书写了。Markdown的理念是能够让文档更容易读、写和修改。HTML是一众发布格式，而Markdown是一种书写格式。
